...
Model_return_contend=["sales_old_past_error","sales_semi_past_error","sales_predict_error_old","sales_predict_error_new","sales_old_past_error_curve_fit","sales_semi_past_error_curve_fit","sales_predict_error_curve_fit","sales_predict_error_new_curve_fit","sales_predict_error_semi"]
...
def sku_classification(Data:pd.DataFrame,start_year:int,end_year:int):
    ...
    Sales_path_head = "../源文件/销量预测误差统计_test/第%d轮滚动结果/相关数据/" % (end_year-2016)
...
def get_Bass_params(sku_all:dict,start_year:int,end_year:int):
    ...
    Sales_path_head_old = "../源文件/销量预测误差统计_test/第%d轮滚动结果/相关数据/Bass_curve_fit图片/老品" % (end_year - 2016)
    Sales_path_head_new = "../源文件/销量预测误差统计_test/第%d轮滚动结果/相关数据/Bass_curve_fit图片/新品" % (end_year - 2016)
...
def Self_attributing_features(roll_num,params_all:dict,self_attributes_all,sku_sales_data_all:dict,start_year,end_year):
    ...
    Model_return_contend_chines = ["老品_历史_销量误差(特征映射)", "新品_历史_销量误差(特征映射)", "老品_预测年份(%d)_销量误差(特征映射)"%(predict_year),"新品_预测年份(%d)_销量误差(特征映射)"%(predict_year),"老品_历史_销量误差(curve_fit)", "新品_历史_销量误差(curve_fit)", "老品_预测年份(%d)_销量误差(curve_fit)"%(predict_year),"新品_预测年份(%d)_销量误差(curve_fit)"%(predict_year),"新品_有销量的预测年份(%d)_销量误差(特征映射)"%(predict_year)]
    ...
    Bass_path_head = "../源文件/Bass参数特征拟合结果_test/第%d轮滚动结果/" % (roll_num)

    train_test_data,scaler=get_train_test(params_all,self_attributes_all,sku_sales_data_all,start_year,end_year)
    ...
    def machine_training(x_train_f,y_train_p,x_train_t,y_train_s,x_test_f ,y_test_s ,x_test_t ,y_test_p,model_name,model,search_space=None,ifInverse=False):
        ...
        sales_old_past_error = get_sale_error(y_train_s, sales_old_predict)
        ...
        sales_old_past_error_curve_fit = get_sale_error(y_train_s, sales_old_init_bass)
        sales_semi_past_error_curve_fit = get_sale_error(y_test_s[semi_brand_have_params], sales_semi_init_bass)
        ...
        sales_predict_error_semi = get_sale_error(y_test_s.loc[predict_year, semi_brand_new_sku],
                                                  sales_new_predict.loc[predict_year, semi_brand_new_sku], predict_year)
    ...
    def RT(x_train_f,y_train_p,x_train_t,y_train_s,x_test_f ,y_test_s ,x_test_t ,y_test_p):#支持向量回归模型
        ...
        n_estimators = [i for i in range(100, 260, 10)]
        search_space = {
...
def sku_sales_past_processing(sku_sales,start_year,end_year,sku_can_use=None,ifBrandNew=False):
    sku_classify_sales: pd.DataFrame
    predict_year=end_year+1
    data:pd.DataFrame
    bool:pd.Series
    ...
    drop_column = []
    self_attribute_new=pd.DataFrame(columns=['As','Asgr'])
    ...
    def get_As_and_Asgr(sku, bool: pd.Series):
        # As :average sales
        ...
        if size > 3:
            ...
            Asgr = (bool.iloc[0] + bool.iloc[2]) / 2
        else:
            if ifBrandNew:
                return pd.Series(name=sku, index=['As', 'Asgr'], data=[0, 0])
            As = np.mean(bool)
            if size == 1:
                Asgr = bool.iloc[0]
            else:
                Asgr = (bool.iloc[0] + bool.iloc[-1]) / (size - 1)
        feature_new = pd.Series(name=sku, index=['As', 'Asgr'], data=[As, Asgr])
        return feature_new
        ...
        for sku in data.columns:
            ...
            if bool.size==0:
                ...
                if sku_can_use is None:
                    ...
                    feature_new=get_As_and_Asgr(sku,bool)
...
def get_train_test(params_all,self_attributes_all,sku_sales_data_all,start_year,end_year):
    # 整合数据 提取老品数据
    ...
    sku_can_ues_old = params_old_init.index
    ...
    sku_old_sales_predict_init, sku_old_t ,new_feature_old= sku_sales_past_processing(sku_sales_data_all["data_sku_old"],
                                                                      start_year, end_year, sku_can_ues_old)
    ...
    # 存数据
    data_save_path="../源文件/测试数据结果/第%d轮/"%(end_year-2016)
...
def sales_plot( sales_past:pd.DataFrame,sales_predict:pd.DataFrame,new_sku,roll_num,Model_name,sales_Bass_init:pd.DataFrame,end_year_init):
    ...
    return_path="../源文件/销量预测误差统计_test/第%d轮滚动结果/%s/%s销量图片"
    return_path_jpg=return_path+"/%s.jpg"
    return_path_data_predict = "../源文件/销量预测误差统计_test/第%d轮滚动结果/%s/%s拟合销量结果(预测数据).csv"
    return_path_data_past= "../源文件/销量预测误差统计_test/第%d轮滚动结果/%s/%s拟合销量结果(原始数据).csv"
    ...
    for sku_name in sales_past.columns:
        ...
        plt.plot(year_get,sku_predict_data,"r*-",alpha=0.8,color="#4ABEA1",label="映射后的拟合数据",linewidth=1)
...
#提取end_year 后一年的销量
def sku_sales_predict_processing(sku_can_use,sku_sales:dict,start_year,end_year):
    sku_classify_sales: pd.DataFrame
    predict_year=end_year+1
    bool:np.ndarray
    data:pd.DataFrame
    ...
    for sku_classify_name, sku_classify_sales in sku_sales.items():
        if not sku_classify_sales.empty :  #DataFrame 不为非空
